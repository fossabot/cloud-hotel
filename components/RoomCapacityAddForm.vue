<template>
  <v-dialog v-model="dialog" persistent max-width="600px">
    <v-tooltip v-if="edit === false" slot="activator" bottom>
      ...
      <span>Add Room Capacity</span>
    </v-tooltip>
    <v-tooltip v-if="edit === true" slot="activator" bottom>
      <v-btn icon flat slot="activator">
        <v-icon>edit</v-icon>
      </v-btn>
      <span>Edit Room Capacity</span>
    </v-tooltip>
    <v-card>
      <v-card-text>
        <v-text-field
...
<script>
  export default {
    ...
    created() {
      if(Object.keys(this.roomCapacity).length > 0) {
        this.form = {
          name: this.roomCapacity.name
        }
        this.edit = true;
      }
    },
    methods: {
      async submit() {
        try {
          ...
          await this.$axios.post('/room-capacities', this.form);
          this.$emit('refresh');
          this.$toast.success("Room capacity create successful")
          this.dialog = false;
          this.loading = false;
          this.form = {
            name: ''
      ...
      async update() {
        try {
          ...
          await this.$axios.$put('/room-capacities/' + this.roomCapacity.id, this.form);
          this.$emit('refresh');
          this.dialog = false;
          this.loading = false;
          this.form = {
