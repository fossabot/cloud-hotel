<?php
    ...
    if (!function_exists('_response')) {
        function _response($results = [], $statusCode = 200)
        {
            return response(compact('results'), $statusCode);
        }
    }
    
    if (!function_exists('_responseError')) {
        function _responseError($errors = [], $type = "token", $statusCode = 400)
        {
            return response(compact('errors', 'type'), $statusCode);
        }
    }
    ...
    if (!function_exists('checkPassword')) {
        function checkPassword(string $value, string $hashedValue, array $options = [])
        {
            return app('hash')
                ->check(
                    $value,
    ...
    if (!function_exists('now')) {
        ...
        {
            $tz = $tz !== null ? $tz : env('APP_TIMEZONE');
